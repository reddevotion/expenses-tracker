...
function ExpensesForm() {
  ...
  return (
    <motion.form action={(formData) => {expenseToEdit ? handleUpdate(formData, expenseToEdit?.id) : createExpense(formData)}} className='w-full mt-8 rounded overflow-hidden' initial={{opacity: 0, y: 100}} animate={{opacity: 1, y: 0}} transition={{delay: 0.1}}>
        ...
        <input type="number" name='amount' placeholder='Amount' className='w-full px-3 py-2 outline-none' defaultValue={expenseToEdit?.amount || 0}/>
